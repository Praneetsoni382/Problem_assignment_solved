...
import { getAssignmentMarksPerQuestion, getCleanAssignmentTitle } from "@/lib/assignment-utils";
...
function GradePage() {
  ...
  const submission = useQuery({
    ...
    queryFn: async () => {
      ...
      const subData = snap.data() as Omit<Submission, "id">;
      let studentData: Profile | null = null;
      if (subData.student_id) {
        const studentSnap = await getDoc(doc(db, "profiles", subData.student_id));
        if (studentSnap.exists()) {
          studentData = { id: studentSnap.id, ...(studentSnap.data() as Omit<Profile, "id">) };
        }
      }
      return {
        id: snap.id,
  ...
  const existingMarks = useQuery({
    ...
    queryFn: async () => {
      const q = query(collection(db, "question_marks"), where("submission_id", "==", submissionId));
  ...
  // Dynamic maximum possible marks calculation (totalQuestions * marksPerQuestion)
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      // Save question marks in Firestore
      for (const row of rows) {
        const markDocId = `${submissionId}_q${row.question_no}`;
        await setDoc(
          doc(db, "question_marks", markDocId),
          {
            id: markDocId,
            ...row,
      ...
      if (studentId && notes.length) {
        for (const note of notes) {
          const noteDocRef = doc(collection(db, "notifications"));
  ...
  async function generateAiFeedback() {
    ...
    try {
      ...
      for (const page of targetPages) {
        ...
        try {
          const resp = await fetch(page.image_url);
          if (resp.ok) {
  ...
  return (
    <AppShell profile={profile ?? null}>
      ...
      {/* Main 2-Column Grading Layout */}
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12 xl:gap-8">
        {/* Left Column: Document Viewer Workspace */}
        <div
          ...
          <div className="mb-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {paperUrl && (
                <QuestionPaperDialog
                  url={paperUrl}
                  title={`${assignment.data?.title || "Assignment"} — Question Paper`}
                  buttonVariant="outline"
                  buttonSize="sm"
                  className="rounded-xl border-[#b8ded4] bg-white/95 text-xs text-[#0d5c52] hover:bg-[#0d5c52]/10 dark:border-slate-700 dark:bg-slate-900"
                />
              )}
        ...
        {/* Right Column: Grading Panel & Feedback System */}
        <div className="flex flex-col space-y-4 lg:col-span-4">
          ...
          {/* Per-Question Marks Inputs List */}
          <div className="space-y-2.5 rounded-2xl border border-border bg-card p-4 shadow-xs dark:bg-card">
            ...
            {/* Dynamic Running-sum helper text */}
            <div className="border-t border-border/60 pt-3 text-xs font-medium text-muted-foreground">
              Running sum: {currentTotal} / {maxPossibleMarks} Total Marks ({totalQuestions}{" "}
              questions × {marksPerQuestion} marks each)
            </div>
