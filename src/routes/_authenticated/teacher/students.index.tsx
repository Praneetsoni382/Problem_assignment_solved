...
import { deleteStudent, listAssignments, listStudents, listSubmissions } from "@/lib/db";
...
export function TeacherStudentsDashboard() {
  ...
  const deleteStudentMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success("Student and all associated submissions permanently deleted from database");
                  ...
                  filteredStudents.map((student) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Action: View Student Vault & Delete Student */}
        ...
        <AlertDialogContent className="rounded-3xl border border-white/60 bg-white/95 p-7 shadow-2xl backdrop-blur-md dark:border-neutral-800 dark:bg-neutral-900/95">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-sm text-muted-foreground">
              ...
              <strong className="text-foreground">{deletingStudent?.name}</strong> (Enrollment:{" "}
              <span className="font-mono">{deletingStudent?.enrollment}</span>)? This will
              completely remove the student profile, all their assignment submissions, scanned
              answer pages, grades, and notifications from the database. This action cannot be
              undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
